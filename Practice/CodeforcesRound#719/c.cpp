#include <bits/stdc++.h>
#include <ext/pb_ds/assoc_container.hpp>

using namespace std;
using namespace __gnu_pbds;

#define vt vector
#define pb push_back
#define all(c) (c).begin(), (c).end()
#define rall(c) (c).rbegin(), (c).rend()
#define sz(x) (int)(x).size()
#define rep(i, a, b) for(int i=a; i<=b; i++)
#define down(i, a, b) for(int i=a; i>=b; i--)
#define dbg(x) cerr << "[" << #x << ": " << (x) << "] ";
#define endln cerr << "\n";
#define fi first 
#define sec second

typedef long long ll;
typedef pair<int, int> ii;
template <class T> using ordered_set = tree<T, null_type, \
less<T>, rb_tree_tag, tree_order_statistics_node_update>; 

template <class K, class V> using ordered_map = tree<K, V, \
less<K>, rb_tree_tag, tree_order_statistics_node_update>;
// find_by_order(k): k-th largest element (from 0)
// order_of_key(x): the number of items that strictly smaller than x

const ll INF = 1e9+1;
const int N = 1e5+1;

/* stuff you should look for
	* WRITE STUFF DOWN
	* DON'T GET STUCK ON ONE APPROACH
*/

int valid(int x, int y, int n){
	if(x > 0 && x <= n && y > 0 && y <= n)
		return 1;
	return 0;
}

int check(vt<vt<int>> arr){
	int check = 1;
	int n = sz(arr[0])-1;
	vt<int> dx{1, 0, 0, -1};
	vt<int> dy{0, 1, -1, 0};
	rep(i, 1, n){
		rep(j, 1, n)
			rep(t, 0, 3)
				if(valid(i+dx[t], j + dy[t], n)){
					if(abs(arr[i+dx[t]][j+dy[t]] - arr[i][j]) == 1)
						return 0;
				}
	}	
	return 1;
}

vt<vt<int>> create(int n){
	vt<vt<int>> ans(n+1, vt<int>(n+1));
	int x = 1, y = 1;
	for(int i= 1; i<=n*n; i+=2){
		ans[x][y] = i;
		++x;
		if(x > n){
			x = 1;
			y++;
		}
	}
	for(int i= 2; i<=n*n; i+=2){
		ans[x][y] = i;
		++x;
		if(x > n){
			x = 1;
			y++;
		}
	}
	return ans;
}

void print(vt<vt<int>> arr){
	int n = sz(arr[0]);
	rep(i, 1, n-1){
		rep(j, 1, n-1)
			cout << arr[i][j] <<" ";
		cout << endl;
	}
}

void solve(){
	int n;
	cin >> n;
	if(n == 2){
		cout << -1 << endl;
	}else{
		vt<vt<int>> arr = create(n);
		print(arr);
	}
}

int main(){
	ios_base::sync_with_stdio(0);
	cin.tie(0);
	cout.tie(0);
	int t; cin >> t;
	while(t--)
	solve();
}
